...
| 📅 | **Оценка** | Фильтр по дате (диапазон). | `filter.date1`, `filter.date2` |
...
- **Активность:** "активный" / "неактивный" (`filter.active`).
- **Оценки:** "только с оценкой" / "только без оценки" (`filter.strictRating`).
...
### График индекса
*   **Шкала:** Горизонтальная ось от -20+ до 20+ с делениями.
*   **Зеленые полоски:** Положительные оценки (успешные встречи).
*   **Черный кружок (центр):** Показывает количество встреч с нейтральной оценкой (число внутри кружка).
*   **Цифры по краям:** Количество негативных (слева) и позитивных (справа) оценок.
...
![Развернутый список встреч](../images/manager/satisfactions/satisfactions_list_expanded.png)
{style="block"}
...
*   **Раскрытие деталей:** Кнопка "Показать"/"Скрыть" переключает видимость истории встреч.