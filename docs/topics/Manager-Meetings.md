...
## 3. Назначение и Редактирование (Drawer)

Боковая панель открывается в двух случаях: при создании новой встречи или при нажатии кнопки "Обработать" / "Карандаш" на существующей.

![Выбор слота](../images/manager/meetings/meeting_slot_picker.png)
{style="block"}

### А. Выбор времени (Slot Picker)
Используется для согласования времени.

1.  **Проверка доступности:** Система автоматически проверяет занятость участников (API `meetingsLog`).
    *   Если у кого-то из участников уже есть встреча в это время, слот будет недоступен (функция `isAvailable`).
2.  **Сетка:** Слоты с шагом 30 минут (11:00 - 17:30).
3.  **Утверждение:** Кнопка "Утвердить встречу" фиксирует время.

### Б. Детали встречи
Если встреча уже создана, в панели также доступны:
*   **AI Rating:** Прогноз ИИ по успешности контакта.
*   **Комментарии:** История заметок менеджеров.
