...
**Код:** `Agents.svelte` (frontend/club-manager/src/views/Agents.svelte)  
...
![Список агентов](../images/manager/agents/agents_list.png)
{style="block"}
...
![Список клиентов агента](../images/manager/agents/agents_clients.png)
{style="block"}

*   **Заголовок:** Отображает общее количество клиентов (счетчик).
...
![Журнал заметок](../images/manager/agents/agents_notes.png)
{style="block"}

*   **История:** Список прошлых заметок отображается выше поля ввода (с прокруткой). Каждая запись содержит дату и имя автора.
...
*   **Заметки:**
    *   `agentNotesList` (Endpoint: `/ma/note/list`) — Получение.
    *   `agentNoteCreate` (Endpoint: `/ma/note/create`) — Создание.
    *   `agentNoteUpdate` (Endpoint: `/ma/note/update`) — Редактирование.

**Пример структуры данных (`agentList`):**
```json
{
  "agents": [
    {
...
### Логика фильтрации
Фильтрация происходит на клиенте (`createAgentsList`) для мгновенного отклика UI.
```typescript
...
### Права доступа
Доступ к разделу имеют роли:
*   `admin`, `chief` — полный доступ ко всем агентам.
*   `manager` — видит агентов, чьи клиенты закреплены за ним (или "общих" агентов).