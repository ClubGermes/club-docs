...
**Код:** `Tags.svelte` (режим `speakers`) (frontend/club-manager/src/views/Tags.svelte)  
...
*   При переходе на `/tags/speakers` (если бы роутер это поддерживал явно) или при ручном выборе фильтров, система активирует поиск именно по вопросам из анкеты спикера (индексы 1-7 в `filterList`).
