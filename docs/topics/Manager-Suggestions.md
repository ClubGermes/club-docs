...
3.  **Индикаторы "Уже знакомы" (Context):**
    *   Справа от аватара могут появляться цветные иконки. Это предупреждение для менеджера:
    *   📞 **Фиолетовая (Contact):** Уже обменялись контактами.
    *   🟢 **Зеленая (Event):** Уже пересекались на мероприятии.
    *   🔵 **Синяя (Online):** Была онлайн-встреча.
    *   *Код:* Объект `suggestionsConnections`.
4.  **Статус (Кружок):**
    *   🔴 **0 (Red):** Отказ / Не интересно.
    *   🟡 **1 (Yellow):** Возможно / В проработке.
    *   🟢 **2 (Green):** Рекомендовано (менеджер берет в работу).

## 3. Детальная проработка (Drawer)

Клик по кнопке "Карандаш" открывает боковая панель.
...
*   **Список:** `suggestionsList` (Endpoint: `/ma/suggestions/control`).
    *   Возвращает:
        ...
        *   `connections`: История знакомств (для индикаторов).
