...
## 5. Управление тарифами и оплатами (Manual Billing)

В текущей версии платформы **отсутствует автоматический биллинг** и интеграция с платежными шлюзами (Yookassa, Stripe и др.).

*   **Процесс:** Все данные об оплатах и продлении тарифов вводятся менеджерами **вручную** через админ-панель (`club-manager`).
*   **Логика:** Система проверяет поле `paid_till` в БД для ограничения доступа, но само списание средств и подтверждение транзакций происходит вне программного комплекса.
*   **Разработка:** При добавлении функционала, завязанного на оплату, следует опираться на текущие поля в таблице `users`, заполняемые менеджерами.
