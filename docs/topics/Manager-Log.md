# 📋 Менеджер: Лог заходов (Log)

**Код:** `Log.svelte` (frontend/club-manager/src/views/Log.svelte)  
**Роут:** `/log` (Layout: `Main`)

Модуль **Лог заходов** предназначен для мониторинга активности пользователей в системе. Он отображает историю сессий (авторизаций) как клиентов мобильного приложения, так и менеджеров/администраторов в веб-интерфейсе.

![Лог заходов](../images/manager/log/log_list.png)
{style="block"}

## 1. Статистика (Сводка)

В правом верхнем углу расположен блок статистики уникальных заходов (Signings Stats).

| Период | Цвет | Переменная в коде |
| :--- | :--- | :--- |
| **Всего** | Серый | `stats['signings_all']` |
| **30 дней** | Зеленый | `stats['signings_month']` |
| **7 дней** | Синий | `stats['signings_week']` |
| **Вчера** | Черный | `stats['signings_yersterday']` |

> **Примечание:** Данные загружаются отдельным запросом `signingsStats` с привязкой к дате "вчера" (`new Date() - 1 day`). В названии переменной `signings_yersterday` сохранена опечатка из исходного кода.

## 2. Фильтрация и Доступ

*   **Фильтр по менеджеру:** Позволяет просматривать активность клиентов, закрепленных за конкретным коммьюнити-менеджером.
*   **Права доступа:**
    *   `admin`, `chief`, `manager`, `moderator` — имеют полный доступ к просмотру лога и фильтрации по любым менеджерам.
    *   `community manager` — видит только активность своих закрепленных клиентов (фильтр заблокирован на его ID).

## 3. Список сессий

Таблица отображает хронологию входов в систему. Данные выводятся страницами по **25 записей**.

### Элементы строки

1.  **Пользователь:**
    *   Аватар и Имя.
    *   **Тип клиента:** Подпись под именем (маппинг `item.settings.client`):
        *   `Приложение` (Мобильный клиент).
        *   `Менеджер` (Веб-панель).
        *   `Модератор`.
    *   **Техническая информация:** Строка User-Agent (устройство, браузер, версия ОС) из `item.settings.agent`, например: `iPhone OS 18_2`.

2.  **Время сессии:**
    *   **Начало:** Дата и время авторизации (`time_from`).
    *   **Конец:** Время завершения активности (`time_to`).

3.  **Статус и Длительность:**
    *   🟢 **Онлайн!** – Если сессия активна (поле `time_to` отсутствует).
    *   ⏱ **Длительность** – Если сессия завершена, рассчитывается разница времени (функция `toTimeLength`), например: "41 минута" или "меньше минуты".

4.  **Действие:**
    *   Кнопка со стрелкой переводит в **Профиль пользователя** (`/users/:id`).

## 4. Техническая реализация

### API Запросы
*   **Список:** `logSignings` (Endpoint: `/ma/log/signings`). Поддерживает пагинацию (`page`) и фильтрацию по `communityManagerId`.
*   **Статистика:** `signingsStats` (Endpoint: `/ma/stats/signings`).

### Хелперы
*   `toTimeLength(seconds)`: Преобразует разницу во времени в человекочитаемую строку ("X час(ов) Y минут").
*   `toFeedDatePart...`: Семейство функций для форматирования дат.

### Особенности
*   Список обновляется при смене страницы пагинации или выборе менеджера в фильтре.
*   Зеленый индикатор "Онлайн" рендерится условно: `{#if item.time_to} ... {:else} Онлайн! {/if}`.
