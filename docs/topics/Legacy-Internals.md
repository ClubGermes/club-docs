...
В проекте используется два разных подхода к изменению схемы данных:

### Новые сервисы (club-events)
Используется инструмент миграций **Aerich** (для Tortoise ORM). Миграции хранятся в папке сервиса и применяются автоматически или через CLI.

### Legacy (club-api)
Для старого монолита **автоматическая система миграций отсутствует**.
*   **Метод:** Все изменения схемы (CREATE TABLE, ALTER TABLE) выполняются **вручную** через SQL-консоль (psql) или GUI-клиенты (DBeaver).
*   **Порядок:** Перед деплоем кода, требующего изменений в БД, разработчик должен самостоятельно применить необходимые SQL-скрипты на продакшн-базе.
*   **Риски:** Отсутствие контроля версий схемы БД требует повышенной осторожности. Рекомендуется сохранять SQL-скрипты изменений в папке `infrastructure/postgres/` для истории.
