...
**Элементы UI в Toolbar:**
*   **Счетчик:** Справа вверху отображается надпись "Найдено: N", показывающая общее количество комментариев, соответствующих фильтру.
*   **Индикаторы (Badges):** При активации фильтра на соответствующей иконке появляется синий кружок с символом бесконечности `∞`.

### Детальная настройка

При нажатии на иконку фильтра открывается панель настройки.

1.  **Фильтр "Клиент":**
    Позволяет искать комментарии по конкретному человеку или компании. Содержит поле текстового ввода и чекбоксы для уточнения поиска.

    ![Фильтр клиента](../images/manager/comments/comments_filter_date.png)
    {style="block"}

2.  **Фильтр "Создание" (Дата):**
    Позволяет выбрать диапазон дат. Появляются два календаря: "С:" и "По:".

    ![Фильтр даты (календари)](../images/manager/comments/comments_filter_date_calendar.png)
    {style="block"}
...
*   **Загрузка:** `ajaxRequest('/comments/fetch')`.
    *   Параметры: `cursor` (ID последнего загруженного комментария), `limit` (обычно 100), `subfilter` (параметры фильтрации).
...
*   **Пагинация:** Реализована классическая постраничная навигация внизу экрана (по **100** записей на страницу), но логика загрузки гибридная (курсор + страница).
