...
**Код:** `Tags.svelte` (frontend/club-manager/src/views/Tags.svelte)  
...
| **События** | Кнопка с иконкой календаря. Позволяет ограничить поиск участниками конкретных мероприятий. | `formType = 'events'` |
| **Кнопка поиска** | Лупа. Запускает поиск. Кнопка неактивна, если запрос не изменился. | `get()` |
| **Настройка полей** | Кнопка с иконкой фильтра. Позволяет выбрать, *где именно* искать (в тегах, вопросах, отраслях). | `formType = 'example'` |
| **Настройка статусов** | Кнопка с иконкой пользователя. Позволяет отфильтровать результаты по статусу клиента. | `formType = 'clients'` |
...
2.  **Контекст (Found in):** Блок справа, показывающий, *почему* этот участник был найден.
    *   Ключевые слова в тексте подсвечиваются **зеленым цветом**.
    *   Данные группируются по категориям: *Текущие поставщики*, *Потребности компании*, *Специализация* и т.д.
    *   Функция `parseData` отвечает за рендеринг и подсветку (`<span class="text-success font-semibold">`).

> **Пагинация:** Список подгружается автоматически при прокрутке ("бесконечный скролл") с помощью `Observer`.
...
*   **Поиск:** `lookup` (Endpoint: `/ma/tag1/lookup`).
    ...
    *   Возвращает список пользователей с контекстом (`tags`, `questions`, `catalog`).
*   **Список событий:** `lookupEvents` (Endpoint: `/ma/tag1/lookup/events`).

### Особенности
*   **Фильтрация на клиенте:** Фильтрация по статусам (`filterStages`) и типам полей (`filter`) применяется **после** получения результатов поиска от сервера (функция `filterUsers`). Это позволяет мгновенно переключать фильтры без повторных запросов к API.
